...
function loadData() {
  try {
    const p = dataPath();
    if (!fs.existsSync(p)) return structuredClone(DEFAULT_DATA);
...
function saveData(todoData) {
  ...
  const tmp = `${p}.tmp`;
  fs.writeFileSync(tmp, JSON.stringify(todoData, null, 2), 'utf8');
  fs.renameSync(tmp, p);
  rebuildReminders(todoData);
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('storage-changed', todoData);
  }
}
